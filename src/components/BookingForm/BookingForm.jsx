...
export default function BookingForm({ availableTimes, dispatch, onSubmit }) {
  ...
  const validate = (e) => {
    ...
    if (!date) errors.date = "Date is invalid";
      ...
      //otherwise, check if the date is generally valid
      else if (year < 1900 || year > 3000) errors.date = "Date is invalid";
        ...
        if (dateParsed.getTime() < today.getTime()) {
          ...
          dateParsed.setHours(Number(time.split(":")[0]));
          dateParsed.setMinutes(Number(time.split(":")[1]));
          if (dateParsed.getTime() < new Date().getTime())
